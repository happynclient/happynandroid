...
public class BootCompletedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        ThreadUtils.mainThreadExecutor(new Runnable() {
            @Override
            public void run() {
                SharedPreferences n2nSp = context.getSharedPreferences("happynet", Context.MODE_PRIVATE);
                // 如果没有配置开机启动,就直接退出
                ...
                N2NSettingModel mCurrentSettingInfo = HappynetApplication.getInstance().getDaoSession().getN2NSettingModelDao().load((long) currentSettingId);
